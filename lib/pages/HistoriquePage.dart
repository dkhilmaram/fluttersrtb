...
class _HistoriquePageState extends State<HistoriquePage>
  ...
  Future<void> _fetchAll() async {
    ...
    final failed = localTickets
        .where((t) => t['statut_sync'] == 'failed' || t['statut_sync'] == 'pending')
        .map(_mapLocalTicket)
        .toList();
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          final onlineOnly = serverList
              .where((t) => !syncedServerIds.contains(t['id_ticket'] as int?))
              .toList();
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Row(children: [
            ...
            SizedBox(width: 8),
            Flexible(child: Text('📡 Hors-ligne — tickets locaux uniquement',
  ...
  Color _tarifColor(String type) {
    ...
    if (t.contains('armée') || t.contains('armee') || t.contains('garde') || t.contains('police')) return const Color(0xFF1E40AF);
    if (t.contains('douane') || t.contains('ministère') || t.contains('ministere'))                return const Color(0xFF374151);
    if (t.contains('municipalité') || t.contains('municipalite') || t.contains('scolaire') ||
  ...
  IconData _tarifIcon(String type) {
    ...
    if (t.contains('armée') || t.contains('armee') || t.contains('garde')) return Icons.shield_rounded;
    if (t.contains('police'))                                      return Icons.local_police_rounded;
    if (t.contains('douane'))                                      return Icons.account_balance_rounded;
    if (t.contains('ministère') || t.contains('ministere'))       return Icons.domain_rounded;
    if (t.contains('municipalité') || t.contains('municipalite')) return Icons.location_city_rounded;
    ...
    if (t.contains('réduit') || t.contains('reduit')) return Icons.discount_rounded;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isLoading
          ? Column(children: [
              _buildSliverHeader(depart, arrivee, asSliverBox: false),
          ...
          : errorMessage != null
              ...
              : NestedScrollView(
                  headerSliverBuilder: (_, __) => [
                    SliverToBoxAdapter(child: _buildSliverHeader(depart, arrivee, asSliverBox: true)),
                  ],
                  body: TabBarView(
                    controller: _tabs,
                    children: [
  ...
  Widget _buildSliverHeader(String depart, String arrivee, {required bool asSliverBox}) {
    return Container(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              // Title + route — FIX: Expanded so the row never overflows
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    // FIX: route row wrapped in individual Flexible texts
                    Row(
                      children: [
                        Container(width: 6, height: 6, decoration: const BoxDecoration(color: goldLight, shape: BoxShape.circle)),
                        ...
                        Padding(
                          ...
                          child: Icon(Icons.arrow_forward, color: Colors.white.withOpacity(0.4), size: 11),
                        ),
                        Container(
                          width: 6, height: 6,
                          decoration: BoxDecoration(
          ...
          if (!isLoading && errorMessage == null)
            Row(
              children: [
                _headerStat(Icons.wifi_rounded,       'En ligne',          '${_onlineTickets.length}',        const Color(0xFF86EFAC)),
                _headerStat(Icons.cloud_done_outlined, 'Sync. hors-ligne', '${_syncedOfflineTickets.length}', Colors.lightBlue.shade200),
                _headerStat(Icons.cloud_off_outlined,  'Échoués',          '${_failedTickets.length}',        Colors.red.shade300),
              ],
            ),
          ...
          if (_syncMessage != null)
            Container(
              ...
              decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
              child: Text(_syncMessage!, style: const TextStyle(color: Colors.white, fontSize: 12)),
            ),

          // ── Tab bar (scrollable to avoid overflow) ──
          TabBar(
            controller: _tabs,
            ...
            tabs: [
              ...
              Tab(text: 'Sync. offline (${_syncedOfflineTickets.length})'),
  ...
  Widget _buildTicketList({
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Container(
                width: 80, height: 80,
                decoration: BoxDecoration(color: navyMid.withOpacity(0.08), shape: BoxShape.circle),
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: tickets.length + (tabType == _TabType.failed ? 1 : 0),
        itemBuilder: (_, i) {
          if (tabType == _TabType.failed && i == 0) {
            return Padding(padding: const EdgeInsets.only(bottom: 16), child: _retryButton());
          }
          final idx  = tabType == _TabType.failed ? i - 1 : i;
          final t    = tickets[idx] as Map<String, dynamic>;
          final prev = idx > 0 ? tickets[idx - 1] as Map<String, dynamic> : null;
          final showDay = prev == null || _formatDay(t['date_heure']) != _formatDay(prev['date_heure']);
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: navyMid.withOpacity(0.2)),
                        ),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            ...
                            Text(_formatDay(t['date_heure']),
                                style: const TextStyle(color: navyMid, fontSize: 12, fontWeight: FontWeight.bold)),
                          ],
                        ),
                      ),
                      const SizedBox(width: 8),
  ...
  Widget _buildTicketCard(Map<String, dynamic> t, _TabType tabType) {
    ...
    switch (tabType) {
      ...
      case _TabType.syncedOffline:
        ...
        badgeLabel  = 'Sync. ✓';          // FIX: shortened to avoid overflow
        borderColor = Colors.lightBlue.shade200;
        break;
      case _TabType.failed:
        final isPending = t['statut_sync'] == 'pending';
        badgeColor  = isPending ? Colors.orange.shade700 : Colors.red.shade600;
        badgeIcon   = isPending ? Icons.cloud_upload_outlined : Icons.cloud_off_outlined;
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: navyMid.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, 3))],
      ),
      child: Column(
        children: [
          // ── Top row ──
          Container(
            ...
            // FIX: wrap entire top row in a Row with no unbounded children
            child: Row(
              children: [
                // Tarif badge — FIX: constrain with Flexible so long names shrink
                Flexible(
                  child: Container(
            ...
            child: Column(
              children: [
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Left side: route + tags
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // Route — FIX: Flexible on both station names + ellipsis
                          Row(
                            children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 6),
                                child: Icon(Icons.arrow_forward, size: 13, color: Colors.grey.shade300),
                              ),
                              Container(
                                width: 7, height: 7,
                                decoration: BoxDecoration(
                          ...
                          Wrap(
                            ...
                            children: [
                              if (t['segment_ordre'] != null)
                                _smallTag(Icons.route, 'Segment ${t['segment_ordre']}', navyMid, navyMid.withOpacity(0.08)),
                              if (t['nom_ligne'] != null)
                                _smallTag(Icons.directions_bus, t['nom_ligne'], Colors.grey.shade500, Colors.grey.shade50),
                              if (tabType != _TabType.online)
                                _smallTag(Icons.storage_rounded, 'Stocké localement', Colors.orange.shade700, Colors.orange.shade50),
                            ],
                          ),
                          if (t['agent'] != null) ...[
                            const SizedBox(height: 6),
  ...
  Widget _smallTag(IconData icon, String label, Color fg, Color bg) {
    return Container(
    ...
    return SizedBox(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Ink(
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [navyDark, navyLight], begin: Alignment.topLeft, end: Alignment.bottomRight),
              borderRadius: BorderRadius.circular(12),
              boxShadow: [BoxShadow(color: navyMid.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))],
            ),
            child: Center(
              child: _isSyncing
