...
class _NouveauTicketPageState extends State<NouveauTicketPage> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      setState(() { errorMessage = 'Impossible de charger les données'; isLoading = false; });
    }
  }
  ...
  List<String> get _departureArrets =>
      arrets.isEmpty ? [] : arrets.sublist(0, arrets.length - 1);
  ...
  Future<void> _saveTicket() async {
    ...
    if (idVente == null) { _showSnack('ID du voyage manquant', isError: true); return; }
    setState(() => isSaving = true);
    final qte = quantite; final prixU = _prixUnitaire!; final prixT = _prixTotal!;
    final dep = pointDepart!; final arr = pointArrivee!; final tarif = typeTarif!;
    try {
      final response = await http.post(
        Uri.parse('http://127.0.0.1:8000/billetterie/tickets/vendre'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'id_vente': idVente, 'id_segment': idSegment ?? 0,
          'point_depart': dep, 'point_arrivee': arr, 'type_tarif': tarif,
          'quantite': qte, 'prix_unitaire': prixU, 'montant_total': prixT,
      ...
      if (data['success'] == true) {
        setState(() {
          ticketsVendus += qte; montantTotal += prixT;
          pointDepart = null; pointArrivee = null; quantite = 1;
        });
        _showSnack(prixT == 0
            ? '✓  $qte passage(s) gratuit(s) enregistré(s)'
            : '✓  $qte ticket(s) · $prixT millimes');
      } else {
        _showSnack('Erreur : ${data['error'] ?? 'inconnue'}', isError: true);
      }
    } catch (e) {
  ...
  Future<void> _cloturerSecteur() async {
    final idVoyage  = widget.voyage['id'] as int?;
    final idSegment = widget.voyage['id_segment'] as int?;
    if (idVoyage == null || idSegment == null) {
      _showSnack('Informations du secteur manquantes', isError: true); return;
    }
    setState(() => isCloturing = true);
    try {
      final clotData = jsonDecode((await http.put(
        Uri.parse('http://127.0.0.1:8000/billetterie/voyages/$idVoyage/segments/$idSegment/cloturer'),
        headers: {'Content-Type': 'application/json'},
      )).body);
      if (clotData['success'] != true) {
        _showSnack(clotData['message'] ?? 'Erreur clôture', isError: true);
        setState(() => isCloturing = false); return;
      }
      _showSnack('Secteur clôturé ✓', isWarning: true);
      final checkData = jsonDecode((await http.get(
        Uri.parse('http://127.0.0.1:8000/billetterie/voyages/$idVoyage/segment/actif'),
      )).body);
      if (checkData['tous_clotures'] == true || checkData['prochain'] == null) {
        if (mounted) Navigator.pop(context);
        setState(() => isCloturing = false); return;
      }
      final openData = jsonDecode((await http.put(
        Uri.parse('http://127.0.0.1:8000/billetterie/voyages/$idVoyage/segment/ouvrir'),
        headers: {'Content-Type': 'application/json'},
      )).body);
      ...
      final newData    = jsonDecode((await http.get(
        Uri.parse('http://127.0.0.1:8000/billetterie/voyages/$idVoyage/segment/actif'),
      )).body);
      final newSecteur = newData['segment'] as Map<String, dynamic>?;
      if (newSecteur == null || !mounted) {
        if (mounted) Navigator.pop(context);
        setState(() => isCloturing = false); return;
      }
      _showSnack('Secteur suivant ouvert ✓');
      Navigator.pushReplacement(context, MaterialPageRoute(
        builder: (_) => NouveauTicketPage(voyage: {
          ...widget.voyage,
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.route, size: 13, color: navyMid),
            ...
            Row(children: [
              ...
              Expanded(
                child: ElevatedButton(
                  onPressed: () { Navigator.pop(context); _cloturerSecteur(); },
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              child: const Icon(Icons.confirmation_number, color: Colors.white, size: 26),
            ),
            const SizedBox(height: 14),
            const Text('Confirmer le ticket',
                style: TextStyle(color: navyDark, fontSize: 17, fontWeight: FontWeight.bold)),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _prixTotal == 0 ? const Color(0xFFDCFCE7) : const Color(0xFFEFF3FF),
                borderRadius: BorderRadius.circular(14),
              ),
              child: Column(children: [
                if (_discountPct > 0)
                  Text('${_prixNormal! * quantite} ms',
                      style: const TextStyle(fontSize: 12, color: Colors.grey,
                          decoration: TextDecoration.lineThrough)),
                if (quantite > 1)
                  Text('$quantite × $_prixUnitaire ms',
                      style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                Text(
                  _prixTotal == 0 ? 'GRATUIT' : '${_prixTotal} millimes',
                  style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold,
                    color: _prixTotal == 0 ? const Color(0xFF16A34A) : navyDark),
                ),
                if (_discountPct > 0) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.orange.shade100, borderRadius: BorderRadius.circular(20),
                    ),
                    child: Text('−$_discountPct% appliqué',
            ...
            Row(children: [
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: navyMid, foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(vertical: 13), elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                  ),
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Flexible(child: Text(value,
            style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: navyDark))),
      ]),
    );
  }
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: navyMid.withOpacity(0.07),
                  blurRadius: 16, offset: const Offset(0, 3))],
            ),
            child: Row(children: [
              Expanded(child: _counterTile(
                Icons.confirmation_number_outlined, 'Tickets vendus',
                '$ticketsVendus', navyMid,
              )),
              Container(width: 1, height: 36, color: Colors.grey.shade100),
              Expanded(child: _counterTile(
                Icons.account_balance_wallet_outlined, 'Total collecté',
            ...
            child: isLoading
                ...
                : errorMessage != null
                    ...
                    : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        // ── 2 columns, 2 rows, horizontal layout ──
                        GridView.builder(
                          shrinkWrap: true,
                          ...
                          gridDelegate:
                            ...
                            crossAxisCount:   2,    // 2 per row → 2 rows for 4 items
                            crossAxisSpacing: 8,
                            mainAxisSpacing:  8,
                            childAspectRatio: 3.8,  // wide & short
                          ),
                          itemBuilder: (_, i) {
                        ...
                        // ── Valider ──
                        _actionBtn(
                          label: isSaving ? 'Enregistrement...'
                              : quantite > 1 ? 'Valider $quantite tickets'
                              : 'Valider le ticket',
  ...
  Widget _label(String text) => Text(text, style: const TextStyle(
      ...
      color: navyDark, letterSpacing: 0.5));
          ...
          DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              icon: Icon(Icons.expand_more,
                  color: onChanged == null ? Colors.grey.shade300 : navyLight.withOpacity(0.5),
                  size: 18),
              style: const TextStyle(color: navyDark, fontSize: 14, fontWeight: FontWeight.w600),
              onChanged: onChanged,
              items: items.map((a) => DropdownMenuItem(
                value: a,
                child: Text(a, style: const TextStyle(color: navyDark, fontSize: 13)),
              )).toList(),
            ),
          ),
        ]),
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Ink(
            decoration: BoxDecoration(
              gradient: enabled
                  ? LinearGradient(colors: colors, begin: Alignment.topLeft,
                      end: Alignment.bottomRight)
                  : null,
...
class _PriceCard extends StatelessWidget {
  ...
  const _PriceCard({
    required this.prixNormal, required this.prixUnitaire,
    required this.prixTotal, required this.quantite, required this.discountPct,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: isFree
            ? const LinearGradient(colors: [Color(0xFFDCFCE7), Color(0xFFF0FDF4)],
                begin: Alignment.topLeft, end: Alignment.bottomRight)
            : const LinearGradient(colors: [Color(0xFFEBF0FF), Color(0xFFF2F5FF)],
                begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
      ...
      child: Column(children: [
        Text(
          isFree ? 'GRATUIT' : '$prixTotal millimes',
          style: TextStyle(
