...
class _NouveauTicketPageState extends State<NouveauTicketPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          await LocalDatabase.saveTarifs(idLigne, data);
          if (mounted) {
            _applyTarifs(data, fromCache: false);
          }
          return;
        }
      }
  ...
  void _applyTarifs(
    ...
    setState(() {
      ...
      prixMap = Map<String, int>.from((data['prix_map'] as Map)
  ...
  Future<void> _saveTicket() async {
    ...
    if (result.success) {
      ...
      if (result.wasOffline) {
        _showSnack('📡 Hors-ligne — ticket sauvegardé localement', isWarning: true);
      } else {
        _showSnack(
          prixT == 0
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                decoration:
                    BoxDecoration(color: surface, borderRadius: BorderRadius.circular(10)),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    Text('$dep  →  $arr',
                        style: const TextStyle(
                            fontWeight: FontWeight.w600, fontSize: 13, color: navyMid)),
                  ],
                ),
              ),
              const SizedBox(height: 10),
      ...
      child: Row(
        children: [
          ...
          Flexible(
            child: Text(value,
                style: const TextStyle(
                    fontWeight: FontWeight.w700, fontSize: 13, color: navyDark)),
          ),
        ],
      ),
    );
...
Widget build(BuildContext context) {
  ...
  Widget _tarifBadge(String text, Color accent, bool isSelected) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isSelected ? Colors.white.withOpacity(0.25) : accent.withOpacity(0.12),
        borderRadius: BorderRadius.circular(20),
      ),
      child: Text(text,
          style: TextStyle(
  ...
  Widget _label(String text) => Text(text,
      style: const TextStyle(
          fontSize: 12, fontWeight: FontWeight.w700, color: navyDark, letterSpacing: 0.5));
  ...
  Widget _card({required Widget child}) => Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        boxShadow: [
          BoxShadow(color: navyMid.withOpacity(0.06), blurRadius: 12, offset: const Offset(0, 3))
        ],
      ),
      child: child);
                ...
                child: DropdownButton<String>(
                  ...
                  items: items
                      .map((a) => DropdownMenuItem(
                            value: a,
                            child: Text(a,
                                style: const TextStyle(color: navyDark, fontSize: 13)),
                          ))
                      .toList(),
                ),
              ),
