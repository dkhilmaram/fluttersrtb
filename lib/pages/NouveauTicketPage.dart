...
class _NouveauTicketPageState extends State<NouveauTicketPage> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final response = await http
          .get(Uri.parse(
              'http://10.19.204.100:8000/billetterie/ligne/$idLigne/tarifs'))
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          if (mounted) _applyTarifs(data, fromCache: false);
          return;
        }
      }
    } catch (_) {}
  ...
  // ─────────────────────────────────────────────────────────────
  // Ticket confirmation dialog — includes QR code
  // ─────────────────────────────────────────────────────────────
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  // ── QR image only — no duplicate chips below ──
                  child: Center(
                    child: QrImageView(
                      data:            qrPayload,
  ...
  Future<void> _saveTicket() async {
    ...
    if (result.success) {
      ...
      _showToast('Erreur : ${result.error ?? 'inconnue'}',
          isError: true);
    }
  ...
  Future<void> _cloturerSecteur() async {
    ...
    try {
      final clotResp = await http
          .put(
            Uri.parse(
                'http://10.19.204.100:8000/billetterie/voyages/$idVoyage/segments/$idSegment/cloturer'),
      ...
      if (clotData['success'] != true) {
        _showToast(clotData['message'] ?? 'Erreur clôture',
            isError: true);
    ...
    if (clotureOk) {
      ...
      try {
        final checkData = jsonDecode(
          (await http
                  .get(Uri.parse(
                      'http://10.19.204.100:8000/billetterie/voyages/$idVoyage/segment/actif'))
                  .timeout(const Duration(seconds: 6)))
              .body,
        );
        ...
        final openData = jsonDecode(
          (await http
                  .put(
                    Uri.parse(
                        'http://10.19.204.100:8000/billetterie/voyages/$idVoyage/segment/ouvrir'),
                    headers: {'Content-Type': 'application/json'},
                  )
                  .timeout(const Duration(seconds: 6)))
              .body,
        ...
        if (openData['success'] != true) {
          _showToast(openData['message'] ?? 'Erreur ouverture',
              isError: true);
          setState(() => isCloturing = false);
          return;
        }
