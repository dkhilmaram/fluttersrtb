...
class _ClotureVoyagePageState extends State<ClotureVoyagePage> {
  ...
  Future<void> _cloturerOnline() async {
    ...
    try {
      // The backend cascade endpoint handles opening + clôturing all
      // segments atomically, so we only need one call here.
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          // ✅ Save with serverStatut='cloture' so future server changes
          //    (e.g. manual reset to 'actif') are detected and respected.
          await LocalDatabase.saveVoyageStatut(
            id,
            'cloture',
            serverStatut: 'cloture',
          );
          await _markAllSegmentsClotureLocally(id);
          _onClotureDone();
        } else {
          setState(() => isCloturing = false);
          if (mounted) {
  ...
  Future<void> _cloturerOffline() async {
    ...
    // The server_statut we last saw is what was passed in voyage.statut
    // (the value the server returned when we loaded the list).
    final lastKnownServerStatut =
        widget.voyage['statut'] as String? ?? 'actif';

    // 1. Mark locally as pending, remembering what the server said last
    await LocalDatabase.saveVoyageStatut(
      id,
      'cloture_pending',
    ...
    // 2. Mark segments locally
    await _markAllSegmentsClotureLocally(id);

    // 3. Queue segment clotures
    await _queuePendingSegmentClotures(id);

    // 4. Queue voyage cloture
  ...
  Future<void> _queuePendingSegmentClotures(int idVente) async {
    final cached = await LocalDatabase.getSegments(idVente);
    if (cached == null) return;

    final segments = cached['segments'] as List<dynamic>? ?? [];
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            shape: BoxShape.circle,
                            border: Border.all(
                                color: navyMid.withOpacity(0.3), width: 2),
                          ),
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton(
                            ...
                            style: OutlinedButton.styleFrom(
                              ...
                              side: BorderSide(
                                  color: navyMid.withOpacity(0.3), width: 1.5),
                              padding:
      ...
      child: Row(
        children: [
          Container(
            ...
            child:
                Icon(Icons.close, color: Colors.red.shade700, size: 12),
          ),
          const SizedBox(width: 10),
          Flexible(
            child: Text(text,
