...
/// Which mode the result card is showing.
enum _ResultMode {
  /// NFC / subscription QR — the existing flow.
  abonnement,

  /// Sold-ticket QR — the new verification flow.
  ticketVendu,
}
...
class _ScanTabPageState extends State<ScanTabPage> {
  ...
  // ── Abonnement result (NFC / subscription QR) ──
  Map<String, dynamic>? _nfcData;

  // ── Sold-ticket result ──────────────────────────
  Map<String, dynamic>? _ticketData;
  _ResultMode _resultMode = _ResultMode.abonnement;

  String? _errorMsg;
  bool    _isSaving     = false;
  int     _totalScanned = 0;

  // ── QR payload cache for 409 fallback ──────────
  // Stores the raw fields parsed from the QR code so that if the server
  // returns 409 with missing ticket data, we can fill in the gaps.
  Map<String, dynamic>? _pendingQrPayload;
  ...
  // ── Helper: get the current voyage's ligne ────────────────
  String get _voyageLigne =>
      ((widget.voyage['nom_ligne'] ?? widget.voyage['ligne'] ?? '') as Object)
          .toString()
          .trim();
  ...
  // ══════════════════════════════════════════════════════════
  // QR scanner — opens camera, returns raw string
  // ══════════════════════════════════════════════════════════

  Future<void> _openBarcodeScanner({required bool forTicket}) async {
    ...
    if (forTicket) {
      await _resolveTicketQr(raw.trim());
    } else {
      await _resolveAbonnementQr(raw.trim());
    }
  }

  // ══════════════════════════════════════════════════════════
  // Abonnement QR — JSON payload, never hits server
  // NO ligne check — abonnement/NFC scan is always allowed
  // ══════════════════════════════════════════════════════════

  Future<void> _resolveAbonnementQr(String raw) async {
    if (!mounted) return;
    setState(() => _scanState = _ScanState.loading);

    try {
      final parsed = Map<String, dynamic>.from(jsonDecode(raw) as Map);
      debugPrint('✅ Abonnement QR JSON parsed successfully');
      await _applyParsedAbonnementData(parsed, mode: _l.scanModeQr);
    } catch (_) {
      debugPrint('❌ Abonnement QR payload is not valid JSON');
      setState(() {
  ...
  // ══════════════════════════════════════════════════════════
  // Sold-ticket QR — handles BOTH formats:
  //   • NouveauTicketPage format: {"id":"ABC123","vente":5,"dep":...}
  //   • Legacy server format:     {"id_ticket": 123}
  //   • Plain integer string:     "123"
  // ══════════════════════════════════════════════════════════

  Future<void> _resolveTicketQr(String raw) async {
    if (!mounted) return;
    setState(() {
      _scanState        = _ScanState.loading;
      _resultMode       = _ResultMode.ticketVendu;
      _pendingQrPayload = null;
    });
    ...
    try {
      ...
      if (parsed.containsKey('id') && parsed['id'] is String) {
        numeroTitre = parsed['id'] as String;
        // ── Cache QR payload so we can fill missing fields on 409 ──
        _pendingQrPayload = parsed;
        debugPrint('🎫 Sold-ticket QR (NouveauTicket format) numero=$numeroTitre');
      } else if (parsed.containsKey('id_ticket')) {
        idTicket = (parsed['id_ticket'] as num?)?.toInt();
        debugPrint('🎫 Sold-ticket QR (legacy format) id=$idTicket');
      } else {
        debugPrint('❌ QR JSON has no recognised ticket key: ${parsed.keys}');
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanTicketQrInvalid;
        });
        return;
      }
    } catch (_) {
      idTicket = int.tryParse(raw);
      if (idTicket == null) {
        debugPrint('❌ QR is not JSON and not an integer: $raw');
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanTicketQrInvalid;
        });
      ...
      debugPrint('🎫 Sold-ticket QR (plain int) id=$idTicket');
    }

    if (numeroTitre != null) {
      await _verifyTicketByNumero(numeroTitre);
    } else if (idTicket != null) {
      await _verifyTicketFromServer(idTicket);
    }
  }

  // ══════════════════════════════════════════════════════════
  // Build a ticket map from a 409 body + optional QR fallback.
  //
  // The backend now ALWAYS sends `ticket` inside the 409 body.
  // If for some reason it's missing, we fall back to the cached
  // QR payload fields so the card never shows all dashes.
  // ══════════════════════════════════════════════════════════

  Map<String, dynamic> _buildTicketFrom409(
    Map<dynamic, dynamic> errBody,
    String ligneInvalideMsg,
  ) {
    Map<String, dynamic> ticket = {};

    // 1) Prefer ticket data embedded by the backend
    if (errBody.containsKey('ticket') && errBody['ticket'] is Map) {
      ticket = Map<String, dynamic>.from(errBody['ticket'] as Map);
      debugPrint('📦 Ticket data found in 409 body');
    }

    // 2) Fill any missing fields from the cached QR payload
    if (_pendingQrPayload != null) {
      ticket['point_depart']  ??= _pendingQrPayload!['dep'];
      ticket['point_arrivee'] ??= _pendingQrPayload!['arr'];
      ticket['type_tarif']    ??= _pendingQrPayload!['tarif'];
      ticket['montant_total'] ??= (_pendingQrPayload!['montant'] as num?)?.toDouble();
      ticket['prix_unitaire'] ??= (_pendingQrPayload!['prix'] as num?)?.toDouble();
      ticket['nom_ligne']     ??= _pendingQrPayload!['ligne'];
      debugPrint('🔄 Merged QR payload fallback fields into 409 ticket');
    }

    ticket['ligne_invalide']     = true;
    ticket['ligne_invalide_msg'] = ligneInvalideMsg.isNotEmpty
        ? ligneInvalideMsg
        : 'Ce ticket n\u2019appartient pas à la ligne de ce voyage.';

    return ticket;
  }

  // ══════════════════════════════════════════════════════════
  // Verify by string numero (NouveauTicketPage QR format)
  // GET /billetterie/tickets/verify-by-numero/{numero}
  //     ?id_voyage_courant=X
  // ══════════════════════════════════════════════════════════

  Future<void> _verifyTicketByNumero(String numero) async {
    final idVoyage = widget.voyage['id'] as int? ?? 0;

    try {
      final uri = Uri.parse(ApiConstants.verifyTicketByNumero(numero))
          .replace(queryParameters: {'id_voyage_courant': '$idVoyage'});

      debugPrint('🔎 Verifying ticket by numero: GET $uri');
      ...
      if (response.statusCode == 404) {
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanTicketNotFound(0);
        });
        return;
      }

      // ── 409: ligne mismatch — backend now includes full ticket data ──
      if (response.statusCode == 409) {
        Map errBody = {};
        try { errBody = jsonDecode(response.body) as Map; } catch (_) {}

        // FastAPI wraps our dict detail in {"detail": {...}}
        final inner = errBody['detail'] is Map
            ? errBody['detail'] as Map
            : errBody;

        final detail = inner['detail']?.toString() ?? '';
        debugPrint('⚠️ HTTP 409 (by numero): $detail');

        final ticket = _buildTicketFrom409(inner, detail);
      ...
      final ticket = Map<String, dynamic>.from(body['ticket'] as Map);

      debugPrint('✅ Ticket verified by numero: $ticket');
      setState(() {
        _scanState  = _ScanState.success;
        _resultMode = _ResultMode.ticketVendu;
        _ticketData = ticket;
      ...
      debugPrint('❌ Ticket verification error (by numero): $e');
      if (mounted) {
        setState(() {
          _scanState = _ScanState.error;
  ...
  // ══════════════════════════════════════════════════════════
  // Verify by integer id (legacy server QR format)
  // GET /billetterie/tickets/{id}/verify
  //     ?id_voyage_courant=X
  // ══════════════════════════════════════════════════════════

  Future<void> _verifyTicketFromServer(int idTicket) async {
    final idVoyage = widget.voyage['id'] as int? ?? 0;

    try {
      final uri = Uri.parse(ApiConstants.verifyTicket(idTicket))
          .replace(queryParameters: {'id_voyage_courant': '$idVoyage'});

      debugPrint('🔎 Verifying ticket by id: GET $uri');
      ...
      if (response.statusCode == 404) {
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanTicketNotFound(idTicket);
        });
        return;
      }
      ...
      if (response.statusCode == 409) {
        ...
        final detail = inner['detail']?.toString() ?? '';
        debugPrint('⚠️ HTTP 409 (by id): $detail');
      ...
      debugPrint('✅ Ticket verified by id: $ticket');
      setState(() {
        _scanState  = _ScanState.success;
        _resultMode = _ResultMode.ticketVendu;
        _ticketData = ticket;
      ...
      debugPrint('❌ Ticket verification error (by id): $e');
      if (mounted) {
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanLookupError(e.toString());
  ...
  // NO ligne check — NFC abonnement scan is always allowed
  // ══════════════════════════════════════════════════════════
  ...
  Future<void> _startNfcScan() async {
    ...
    setState(() => _resultMode = _ResultMode.abonnement);
  ...
  Future<void> _resolveNfcByUid(String uid) async {
    ...
    if (isOnline) {
      try {
        ...
        await _applyParsedAbonnementData(card, mode: _l.scanModeNfc);
      } catch (e) {
  ...
  Future<void> _resolveNfcFromLocal(String uid) async {
    try {
      ...
      await _applyParsedAbonnementData(
        {
          'id':        uid,
  ...
  // ══════════════════════════════════════════════════════════
  // Apply parsed abonnement data (shared by NFC and sub QR)
  // ── NO ligne check here — abonnement/NFC is always allowed ──
  // ══════════════════════════════════════════════════════════

  Future<void> _applyParsedAbonnementData(
    ...
    final expire    = _normaliseIsoDate(data['expire']?.toString() ?? '');
    ...
    setState(() {
      _scanState  = _ScanState.success;
      _resultMode = _ResultMode.abonnement;
      _nfcData    = {
        'mode':      mode,
        'id':        id,
        'nom':       nom,
        ...
        // No isLigneInvalide — abonnement/NFC has no ligne restriction
      };
    });
  }
  ...
  static String _normaliseIsoDate(String raw) {
    if (raw.isEmpty) return '';
    if (raw.contains('%') || raw.length < 8) return '';
    final datePart = raw.trim().split(RegExp(r'[ T]')).first;
    if (!RegExp(r'^\d{4}-\d{2}-\d{2}$').hasMatch(datePart)) return '';
    return datePart;
  }

  // ── Expiry check ──────────────────────────────────────────

  static bool _checkExpired(String isoDate) {
    ...
    try {
      ...
      return expireDay.isBefore(DateTime(now.year, now.month, now.day));
    } catch (_) {
      return false;
    }
  }
  ...
  // ══════════════════════════════════════════════════════════
  // Save — abonnement flow (offline-first, unchanged)
  // ══════════════════════════════════════════════════════════

  Future<void> _validerAbonnement() async {
  ...
  // ══════════════════════════════════════════════════════════
  // Validate sold ticket — POST /tickets/{id}/scan
  //                     OR POST /tickets/by-numero/{numero}/scan
  // ══════════════════════════════════════════════════════════

  Future<void> _validerTicketVendu() async {
    if (_ticketData == null) return;

    final alreadyScanned = _ticketData!['already_scanned'] as bool? ?? false;
    final ligneInvalide  = _ticketData!['ligne_invalide']  as bool? ?? false;
    if (alreadyScanned || ligneInvalide) return;

    setState(() => _isSaving = true);

    final idVoyage    = widget.voyage['id'] as int? ?? 0;
    final numeroTitre = _ticketData!['numero_titre'] as String?;
    final idTicket    = _ticketData!['id_ticket']    as int?;

    try {
      late http.Response response;

      if (numeroTitre != null && numeroTitre.isNotEmpty) {
        final uri = Uri.parse(ApiConstants.scanTicketByNumero(numeroTitre));
        debugPrint('📤 Scanning ticket by numero: POST $uri');
        response = await http
            .post(
              uri,
              headers: {'Content-Type': 'application/json'},
              body: jsonEncode({'id_voyage_courant': idVoyage}),
            )
            .timeout(ApiConstants.defaultTimeout);
      } else if (idTicket != null) {
        final uri = Uri.parse(ApiConstants.scanTicket(idTicket));
        debugPrint('📤 Scanning ticket by id: POST $uri');
        response = await http
            .post(
              uri,
              headers: {'Content-Type': 'application/json'},
        ...
        throw Exception('No ticket identifier available');
      }
      ...
      if (response.statusCode == 409) {
        Map body = {};
        try { body = jsonDecode(response.body) as Map; } catch (_) {}
        final detail = (body['detail'] is Map
            ? body['detail']['detail']
            : body['detail'])
            ?.toString() ?? _l.scanTicketAlreadyScanned;
        setState(() {
          _isSaving  = false;
          _scanState = _ScanState.error;
          _errorMsg  = detail;
      ...
      _showToast(_l.scanTicketValidatedToast);
      setState(() {
        _isSaving         = false;
        _totalScanned++;
        _scanState        = _ScanState.idle;
      ...
      debugPrint('❌ Ticket scan error: $e');
      if (mounted) {
        setState(() => _isSaving = false);
        _showToast(_l.scanSaveError(e.toString()), isError: true);
      }
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (showScanBtns) ...[
              ...
              children: [
                ...
                Expanded(
                  child: _ScanModeBtn(
                    ...
                    onTap: _scanState == _ScanState.idle
                        ? () => _openBarcodeScanner(forTicket: false)
                        : null,
                  ),
                ),
              ],
            ...
            // ── Row 2: Sold-ticket QR (full width) ─────────
            _ScanModeBtn(
              icon: Icons.confirmation_number_rounded,
              label: l.scanModeTicketVendu,
              sublabel: l.scanTicketVenduSublabel,
              color: const Color(0xFF0F766E),
              onTap: _scanState == _ScanState.idle
                  ? () => _openBarcodeScanner(forTicket: true)
                  : null,
            ),
          ...
          AnimatedSwitcher(
            ...
            child: switch (_scanState) {
              ...
              _ScanState.success => _resultMode == _ResultMode.ticketVendu
                  ? _TicketVenduResultWidget(
                      key: const ValueKey('ticket-success'),
                      data:       _ticketData!,
                      isSaving:   _isSaving,
                      onCancel:   _reset,
                      onValidate: _validerTicketVendu,
                      l: l,
                    )
                  : _NfcResultWidget(
                      key: const ValueKey('abonnement-success'),
                      data:       _nfcData!,
                      isSaving:   _isSaving,
                      onCancel:   _reset,
                      onValidate: _validerAbonnement,
                      l: l,
                    ),
...
// ═════════════════════════════════════════════════════════════
// _TicketVenduResultWidget — verification card for sold tickets
// ═════════════════════════════════════════════════════════════

class _TicketVenduResultWidget extends StatelessWidget {
  final Map<String, dynamic> data;
  final bool isSaving;
  final VoidCallback onCancel, onValidate;
  final AppLocalizations l;

  const _TicketVenduResultWidget({
    super.key,
    required this.data,
  ...
  String _formatPrice(double v) {
    ...
    final str = v.toStringAsFixed(v.truncateToDouble() == v ? 0 : 3);
    final parts = str.split('.');
    final intPart = parts[0].replaceAllMapped(
      RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'),
      (m) => '${m[1]} ',
    );
    return parts.length > 1 ? '$intPart.${parts[1]} DT' : '$intPart DT';
  }

  @override
  Widget build(BuildContext context) {
    final alreadyScanned   = data['already_scanned']   as bool?   ?? false;
    final ligneInvalide    = data['ligne_invalide']     as bool?   ?? false;
    final ligneInvalideMsg = data['ligne_invalide_msg'] as String? ?? '';
    final isInvalid        = alreadyScanned || ligneInvalide;

    final idTicket  = data['id_ticket'];
    final pointDep  = data['point_depart']  as String? ?? '—';
    final pointArr  = data['point_arrivee'] as String? ?? '—';
    final typeTarif = data['type_tarif']    as String? ?? '—';
    final quantite  = data['quantite']      as int?    ?? 1;
    final prixUnit  = (data['prix_unitaire'] as num?)?.toDouble() ?? 0.0;
    final montant   = (data['montant_total'] as num?)?.toDouble()  ?? 0.0;
    final dateHeure = data['date_heure']    as String? ?? '';
    final nomLigne  = data['nom_ligne']     as String? ?? '—';
    final agentNom  = data['agent_nom']     as String? ?? '—';
    final scannedAt = data['scanned_at']    as String?;

    final accent       = isInvalid ? Colors.red.shade600      : const Color(0xFF0F766E);
    final accentLight  = isInvalid ? Colors.red.shade50       : const Color(0xFFCCFBF1);
    final accentBorder = isInvalid ? Colors.red.shade200      : const Color(0xFF5EEAD4);
    final accentText   = isInvalid ? Colors.red.shade800      : const Color(0xFF0F5352);
    final accentSub    = isInvalid ? Colors.red.shade400      : const Color(0xFF14B8A6);

    String statusLabel;
    if (alreadyScanned)    statusLabel = l.scanStatusAlreadyScanned;
    else if (ligneInvalide) statusLabel = 'Ligne invalide';
    else                    statusLabel = l.scanStatusValid;

    String btnLabel;
    if (isSaving)           btnLabel = l.scanSaving;
    else if (alreadyScanned) btnLabel = l.scanTicketAlreadyScannedBtn;
    else if (ligneInvalide)  btnLabel = 'Ligne invalide';
    else                     btnLabel = l.scanBtnValidate;
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                _ScanField(
                  label: l.scanFieldTarif,
                  child: Text(typeTarif,
                      style: const TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w700,
                          color: _navyDark)),
                ...
                _ScanField(
                  label: l.scanFieldPrixUnitaire,
                  child: Text(_formatPrice(prixUnit),
                      style: const TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w700,
                          color: _navyDark)),
                ...
                _ScanField(
                  label: l.scanFieldMontantTotal,
                  child: Text(
                    _formatPrice(montant),
                    style: TextStyle(
                      fontSize: 13,
                ...
                _ScanField(
                  label: l.scanFieldDateVente,
                  child: Text(_formatDateTime(dateHeure),
                      style: const TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
                          color: _navyDark)),
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      scannedAt != null
                          ? l.scanTicketAlreadyScannedAt(_formatDateTime(scannedAt))
                          : l.scanTicketAlreadyScanned,
                      style: TextStyle(
                          fontSize: 12,
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: (isInvalid || isSaving) ? null : onValidate,
                    icon: isSaving
                        ? const SizedBox(
                            width: 16, height: 16,
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      disabledBackgroundColor: isInvalid
                          ? Colors.red.shade200
...
// isLigneInvalide removed — abonnement/NFC has no ligne restriction
// ═════════════════════════════════════════════════════════════
...
class _NfcResultWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Only check expiry — NO ligne check for abonnement/NFC
              ...
              children: [
                _ScanField(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(
                      color: ts.bg, borderRadius: BorderRadius.circular(20)),
                    child: Text(type,
                        style: TextStyle(
                            fontSize: 11,
                            fontWeight: FontWeight.w700,
                ...
                _ScanField(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _navyMid, borderRadius: BorderRadius.circular(6)),
                    alignment: Alignment.center,
                    child: Text(ligne,
                        style: const TextStyle(
                            color: Colors.white,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(l.scanExpiredWarning,
                        style: TextStyle(
                            fontSize: 12,
                            color: Colors.red.shade700,
                            fontWeight: FontWeight.w600)),
