...
class _ScanTabPageState extends State<ScanTabPage> {
  ...
  Future<void> _openBarcodeScanner() async {
    ...
    if (raw == null) return;
    await _resolveCode(raw.trim(), _l.scanModeQr);
  }
  ...
  Future<void> _startNfcScan() async {
    ...
    if (!available) {
      if (mounted) _showToast(_l.scanNfcUnavailable, isWarning: true);
      return;
    }
    if (!mounted) return;
    ...
    NfcManager.instance.startSession(
      onDiscovered: (NfcTag tag) async {
        try {
          ...
          debugPrint('🔍 NFC raw extracted: "$raw"');
          ...
          if (raw == null || raw.trim().length < 2) {
            ...
            await _resolveCode(raw.trim(), _l.scanModeNfc);
          }
        } catch (e) {
  ...
  // ── Extract raw string from NFC tag ──────────────────────
    ...
    if (ndef != null && ndef.cachedMessage != null) {
      for (final record in ndef.cachedMessage!.records) {
        if (record.typeNameFormat == NdefTypeNameFormat.nfcWellknown &&
          ...
          if (payload.length > 1) {
            ...
            if (payload.length > 1 + langLen) {
              ...
              debugPrint('📄 NDEF Text record: "$text"');
        ...
        if (record.typeNameFormat == NdefTypeNameFormat.nfcWellknown &&
          ...
          final payload = record.payload;
          if (payload.length > 1) {
            final text = utf8.decode(payload.sublist(1), allowMalformed: true).trim();
            debugPrint('📄 NDEF URI record: "$text"');
            if (text.isNotEmpty) return text;
          }
        }
        ...
        try {
          ...
          if (text.isNotEmpty) {
            debugPrint('📄 NDEF generic record: "$text"');
            return text;
          }
        } catch (_) {}
      }
  ...
  Future<void> _resolveCode(String raw, String mode) async {
    if (!mounted) return;
    ...
    if (parsed != null) {
      debugPrint('✅ Resolved as JSON payload');
      _applyParsed(parsed, mode);
      return;
    }

    final normalised = raw.toUpperCase();
    debugPrint('🔎 Looking up card ID in DB: "$normalised"');
    await _resolveByCardId(normalised, mode);
  }

  void _applyParsed(Map<String, dynamic> parsed, String mode) {
    final id        = parsed['id']?.toString()         ?? '';
    final nom       = parsed['nom']?.toString()        ?? _l.scanUnknown;
    final type      = parsed['type']?.toString()       ?? _l.scanUnknown;
  ...
  Future<void> _resolveByCardId(String cardId, String mode) async {
    try {
      final ticket = await TicketDao.findByCardId(cardId);

      if (!mounted) return;

      if (ticket == null) {
        debugPrint('❌ Card not found in DB for ID: "$cardId"');
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanCardNotFound(cardId);
        });
        return;
      }

      debugPrint('✅ Card found in DB: $ticket');

      final expire    = ticket['expire']?.toString()    ?? '';
      final nom       = ticket['nom']?.toString()       ?? _l.scanUnknown;
      final type      = ticket['type']?.toString()      ?? _l.scanUnknown;
      final ligne     = ticket['ligne']?.toString()     ?? '—';
      final organisme = ticket['organisme']?.toString() ?? '—';
  ...
  Future<void> _validerEtVendre() async {
    ...
    if (_scannedData!['isExpired'] == true) return;
    ...
    final result = await TicketRepository.saveTicket({
      ...
      'type_tarif':      '${_l.scanPrefix} ${_scannedData!['mode']} — ${_scannedData!['type']}',
      'quantite':        1,
      'prix_unitaire':   0,
      'montant_total':   0,
      'matricule_agent': widget.voyage['matricule_agent'] ?? 0,
      'numero_titre':    _scannedData!['id'],
      'nom_titulaire':   _scannedData!['nom'],
      'organisme':       _scannedData!['organisme'],
      'ligne_titre':     _scannedData!['ligne'],
    });
        ...
        children: [
            ...
            children: [
              Expanded(
                child: _ScanModeBtn(
                  ...
                  onTap: _scanState == _ScanState.idle ? _startNfcScan : null,
                ),
              ),
              ...
              Expanded(
                child: _ScanModeBtn(
                  ...
                  onTap: _scanState == _ScanState.idle ? _openBarcodeScanner : null,
                ),
              ),
            ],
          ),
          ...
          AnimatedSwitcher(
            ...
            child: switch (_scanState) {
              _ScanState.idle    => _IdleWidget(key: const ValueKey('idle'), l: l),
              _ScanState.loading => _LoadingWidget(key: const ValueKey('loading'), l: l),
