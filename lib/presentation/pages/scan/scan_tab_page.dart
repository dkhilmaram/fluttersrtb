...
class ScanTabPage
  ...
  State<
    ScanTabPage
  >
  createState() => _ScanTabPageState();
}

class _ScanTabPageState
    extends
        State<
          ScanTabPage
        > {
  ...
  void _resolveCode(
    ...
    final type =
        ...
        'Inconnu';
    final expire =
        parsed['expire']?.toString() ??
        '';
    final ligne =
        parsed['ligne']?.toString() ??
        '—';
    final organisme =
        parsed['organisme']?.toString() ??
        '—';
        ...
        children: [
          ...
          AnimatedSwitcher(
            ...
            child: switch (scanState) {
              _ScanState.idle => const _IdleWidget(
                key: ValueKey(
                  'idle',
                ),
              ),
...
class _NfcListeningSheet
  ...
  State<
    _NfcListeningSheet
  >
  createState() => _NfcListeningSheetState();
}

class _NfcListeningSheetState
    extends
        State<
          _NfcListeningSheet
        >
...
class _CameraScannerPage
  ...
  State<
    _CameraScannerPage
  >
  createState() => _CameraScannerPageState();
}

class _CameraScannerPageState
    extends
        State<
          _CameraScannerPage
        > {
...
class _ScanResultWidget
  ...
  Widget build(
    ...
    final accentLight = isExpired
        ? Colors.red.shade50
        : const Color(
            0xFFDCFCE7,
          );
    final accentBorder = isExpired
        ? Colors.red.shade200
        : const Color(
            0xFF86EFAC,
          );
    final accentText = isExpired
        ? Colors.red.shade800
        : const Color(
            0xFF15803D,
          );
    final accentSub = isExpired
        ? Colors.red.shade400
        : const Color(
            0xFF4ADE80,
          );
