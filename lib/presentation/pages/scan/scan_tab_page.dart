...
class _ScanTabPageState extends State<ScanTabPage> {
  ...
  Future<void> _resolveTicketQr(String raw) async {
    ...
    String? numeroTitre; // string ID from NouveauTicketPage QR  → uses verifyTicketByNumero
    int?    idTicket;    // integer ID from legacy server QR      → uses verifyTicket

    try {
      final parsed = jsonDecode(raw) as Map<String, dynamic>;

      // ── Format generated by NouveauTicketPage ─────────────
      // {"id": "XXX", "vente": 5, "dep": "...", "arr": "...", ...}
      ...
      // ── Legacy format from server-side QR generation ──────
      // {"id_ticket": 123}
      ...
      // ── Any other JSON — not a valid sold-ticket QR ───────
      else {
      ...
      // Not JSON — maybe it's a raw integer string
  ...
  Future<void> _verifyTicketByNumero(String numero) async {
    ...
    try {
      ...
      if (response.statusCode == 409) {
        ...
        setState(() {
          _scanState = _ScanState.error;
          _errorMsg  = _l.scanTicketLigneMismatch(
  ...
  // ══════════════════════════════════════════════════════════
  // NFC scanner — UID extracted → server lookup (unchanged)
  // ══════════════════════════════════════════════════════════
  ...
  Future<void> _validerTicketVendu() async {
    ...
    // Prefer the string numero_titre if present (NouveauTicketPage flow),
    // otherwise fall back to the integer id_ticket (legacy flow).
    ...
    try {
      ...
      if (response.statusCode == 409) {
        ...
        setState(() {
          _isSaving  = false;
          _scanState = _ScanState.error;
          _errorMsg  = body['detail']?.toString() ?? _l.scanTicketAlreadyScanned;
        });
        return;
      }
...
class _TicketVenduResultWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final accent       = alreadyScanned ? Colors.red.shade600      : const Color(0xFF0F766E);
    final accentLight  = alreadyScanned ? Colors.red.shade50       : const Color(0xFFCCFBF1);
    final accentBorder = alreadyScanned ? Colors.red.shade200      : const Color(0xFF5EEAD4);
    final accentText   = alreadyScanned ? Colors.red.shade800      : const Color(0xFF0F5352);
    final accentSub    = alreadyScanned ? Colors.red.shade400      : const Color(0xFF14B8A6);
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: (alreadyScanned || isSaving) ? null : onValidate,
                    icon: isSaving
                        ? const SizedBox(
                            width: 16, height: 16,
                    ...
                    label: Text(
                      isSaving
                          ? l.scanSaving
                          : alreadyScanned
                              ? l.scanTicketAlreadyScannedBtn
                              : l.scanBtnValidate,
                      style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 13),
                    ),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: alreadyScanned
                          ? Colors.red.shade400
                          : const Color(0xFF0F766E),
                      foregroundColor: Colors.white,
                      disabledBackgroundColor: alreadyScanned
                          ? Colors.red.shade200
...
// ═════════════════════════════════════════════════════════════
// _NfcResultWidget — rich subscriber card (QR + NFC, unchanged)
// ═════════════════════════════════════════════════════════════
