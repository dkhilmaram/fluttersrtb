...
class NouveauTicketPageState
  ...
  void resetAfterGratuit() {
    ...
    setState(
      () {
        if (currentDepart !=
            null) {
          final usedIndex = arrets.indexOf(
            currentDepart,
          );
          if (usedIndex >=
              0)
  ...
  _fetchData() async {
    ...
    final cached = await VoyageDao.getTarifs(
      idLigne,
    );
    if (cached !=
        null) {
      if (segmentArrets ==
          ...
          idVente !=
              null) {
        // FIX #1: delegate to VoyageDao
        final segCache = await VoyageDao.getSegments(
          idVente,
        );
        if (segCache !=
            null) {
  ...
  void _applyTarifs(
    ...
    final orderedArrets = _orderArretsByDirection(
      rawArrets,
    );
    final defaultDepart = orderedArrets.isNotEmpty
        ? orderedArrets.first
        : null;
  ...
  List<
    String
  >
  get _departureArrets {
    if (arrets.isEmpty) return [];
    final from = _minDepartureIndex.clamp(
      0,
      arrets.length -
          1,
    );
  ...
  Widget build(
    ...
    final body = SingleChildScrollView(
      child: Column(
        children: [
          // Standalone header (hidden in embedded mode)
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Flexible(
                                child: Text(
                                  'Mode hors-ligne — les actions seront '
                                  'synchronisées dès la reconnexion',
                                  style: TextStyle(
                                    fontSize: 12,
                                    color: Colors.orange.shade700,
                                  ),
