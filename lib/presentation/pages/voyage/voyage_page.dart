...
// ─────────────────────────────────────────────────────────────
const String
_kReportRecipient = 'khalilbenritha@gmail.com';
const String
_kSmtpHost = 'smtp.gmail.com';
const int
_kSmtpPort = 587;
const String
_kSmtpUser = 'dkhilmaram0@gmail.com';
const String
_kSmtpPassword = 'ppax xarr sfwc wejn';
// ─────────────────────────────────────────────────────────────
...
void _showToast(
  ...
  _mergeLocalStatuts(
    ...
    for (final v in voyages) {
      ...
      if (idVente !=
          null) {
        final reopenPending = await VoyageDao.isReopenPending(
          idVente,
        );
        if (reopenPending) {
          voyage['statut'] = 'actif';
  ...
  _fetchProgrammes() async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse(
              ApiConstants.voyagesProgrammes(
                _matricule,
              ),
            ),
            headers: {
        ...
        final list =
            jsonDecode(
                ...
                as List<
                  dynamic
                >;
        await VoyageDao.clearStaleVoyageStatuts(
          list,
        );
        await VoyageDao.saveVoyages(
          _matricule,
          list,
        );
    ...
    final cached = await VoyageDao.getVoyages(
      _matricule,
    );
    if (cached !=
        null) {
  ...
  _fetchNonProgrammes() async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse(
              ApiConstants.voyagesNonProgrammes(
                _matricule,
              ),
            ),
            headers: {
 ...
 void _maybeShowOfflineToast() {
  ...
  _clotureJourneeAll() async {
    ...
    if (offline) {
      for (final id in ids) {
        await VoyageDao.saveCloturePending(
          id,
        );
  ...
  Future<
    void
  >
  _reopenJourneeAll() async {
    final t = AppLocalizations.of(
      context,
    )!;
    final allClotures = [
      ...voyagesProgrammes.where(
        (
          v,
        ) =>
            v['statut'] ==
    ...
    if (offline) {
      for (final id in ids) {
        ...
        await VoyageDao.saveReopenPending(
          id,
          scope: 'journee',
        );
      }
    ...
    try {
      ...
      for (final v in allVoyages) {
        ...
        if (id ==
            null)
          continue;
        final rows = await TicketDao.getTicketsByVoyage(
          id,
        );
        for (final r in rows) {
          final ticket =
  ...
  _buildExcel({
    ...
    void cell(
      ...
      if (value
          is int) {
        c.value = xl.IntCellValue(
          value,
        );
      } else if (value
          is double) {
        c.value = xl.DoubleCellValue(
          value,
        );
      } else {
        c.value = xl.TextCellValue(
    ...
    for (final e in segEntries) {
      ...
      for (final t in e.value) {
        ...
        tarifInSeg[type]!['count'] =
            tarifInSeg[type]!['count']! +
            qty;
        if (tot ==
            0)
          tarifInSeg[type]!['gratuits'] =
              tarifInSeg[type]!['gratuits']! +
              qty;
        tarifInSeg[type]!['recette'] =
            tarifInSeg[type]!['recette']! +
            tot;
      }
...
'''
  ...
  Widget _buildSharedActionBar() {
    ...
    if (voyagesProgrammes.isEmpty &&
        voyagesNonProgrammes.isEmpty)
      return const SizedBox.shrink();
    return Container(
  ...
  Widget _buildClotureConfirmCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppTheme.navyMid.withOpacity(
          0.5,
        ),
